...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        c, s, e, n = self._backfill_bounty_payouts(dry_run, batch_size)
        total_created += c
        total_skipped += s
        total_errors += e
        ...
        c, s, e, n = self._backfill_document_tips(dry_run, batch_size)
        total_created += c
        total_skipped += s
        total_errors += e
        total_would_process += n if dry_run else 0
        ...
        c, s, e, n = self._backfill_review_tips(dry_run, batch_size)
        total_created += c
        total_skipped += s
        total_errors += e
        total_would_process += n if dry_run else 0
        ...
        c, s, e, n = self._backfill_fees(dry_run, batch_size)
        total_created += c
        total_skipped += s
        total_errors += e
        total_would_process += n if dry_run else 0
    ...
    def _backfill_fundraise_payouts(self, dry_run, batch_size):
    ...
    def _backfill_bounty_payouts(self, dry_run, batch_size):
    ...
    def _backfill_document_tips(self, dry_run, batch_size):
        label = "document_tips"
    ...
    def _backfill_review_tips(self, dry_run, batch_size):
        label = "review_tips"
    ...
    def _backfill_fees(self, dry_run, batch_size):
        label = "fees"
