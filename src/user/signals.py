...
def create_action(sender, instance, created, **kwargs):
    if created:
        ...
        if user is not None and user.invited_by and not Action.objects.filter(user=user, content_type__in=referral_content_types).exists() and sender in [Thread, Reply, Comment, Paper]:
            timestamp = time()
