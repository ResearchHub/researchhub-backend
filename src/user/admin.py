...
class AnalyticAdminPanel(admin.ModelAdmin):
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path("chart_data/", self.admin_site.admin_view(self.chart_data_viewset)),
        ]
    ...
    def get_actions_dataframe(self, start_date, end_date):
        actions = (
            ...
            .annotate(date=models.functions.TruncDate("created_date"))
        )
        actions_created_dates = actions.values_list("date", flat=True)
        actions_user_ids = actions.values_list("user", flat=True)
    ...
    def process_data(self, start_date, end_date, days_range, chart_selector):
        if chart_selector == "day":
            ...
            normalized_dates = actions_df["created_date"] + pd.DateOffset(
                days=days_offset
            )
            ...
            unique_contributors_df = unique_con_df[["created_date", 0]].set_index(
                "created_date"
            )
            ...
            actions_df = self.get_actions_dataframe(start_date, end_date)
            unique_con_df = (
                actions_df.groupby(pd.Grouper(key="created_date", freq="M"))
                .apply(self.get_unique_users)
                .to_frame()
                .reset_index()
            )
        ...
        for date, user_data in zip(
            unique_contributors_df.index, unique_contributors_df.values
        ):
    ...
    def chart_data_viewset(self, request):
        ...
        end_date = request.GET.get(
            "end_date", datetime.datetime.now().strftime("%Y-%m-%dT%H:%M")
        )
        days_range = request.GET.get("days_range", "1")
        chart_selector = request.GET.get("chart_selector", "day")
        start_date = datetime.datetime.strptime(start_date, "%Y-%m-%d")
        end_date = datetime.datetime.strptime(end_date, "%Y-%m-%dT%H:%M")

        res = self.process_data(start_date, end_date, days_range, chart_selector)
...
class VerificationAdminPanel(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        unique = (
            Verification.objects.order_by("user_id", "id")
            .distinct(
                "user_id",
            )
            .values_list("id")
