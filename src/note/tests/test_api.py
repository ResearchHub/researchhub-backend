...
class NoteTests(APITestCase):
    ...
    def test_note_editor_can_update_contents(self):
        ...
        # Update title
        response = self.client.patch(f"/api/note/{note['id']}/", {"title": "some title"})
        ...
        # Update body
        response = self.client.post(f"/api/note_content/", {"full_src": "updated body", "note": note['id'], "plain_text": ""})
    ...
    def test_note_viewer_cannot_update_contents(self):
        ...
        # Update title
        response = self.client.patch(f"/api/note/{note['id']}/", {"title": "updated title"})
        self.assertEqual(response.status_code, 403)
    ...
    def test_removing_note_org_access_makes_note_private(self):
        ...
        self.client.delete(f"/api/note/{note['id']}/remove_permission/", {"organization": self.org["id"]})
