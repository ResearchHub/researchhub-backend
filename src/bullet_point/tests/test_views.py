...
class BulletPointViewsTests(TestCase):
    def setUp(self):
        ...
        self.bullet_point = create_bullet_point(paper=self.paper, created_by=self.user)

    def test_user_can_update_ordinal(self):
        data = {"ordinal": 1}
        response = self.get_patch_response(self.user, self.bullet_point.id, data)
    ...
    def test_edit_creates_new_bullet_point(self):
        ...
        response = self.get_edit_response(self.user, self.bullet_point.id, data)
    ...
    def get_lock_ordinal_response(self, user, bullet_point_id, lock=True):
        ...
        return self.get_patch_response(user, bullet_point_id, data)
