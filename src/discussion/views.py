...
from .utils import get_comment_id_from_path, get_thread_id_from_path
...
class ThreadViewSet(viewsets.ModelViewSet, ReactionViewActionMixin):
    ...
    def get_queryset(self):
        upvotes = Count(
            "votes",
            filter=Q(
                votes__vote_type=Vote.UPVOTE,
            ),
        )
        downvotes = Count(
            "votes",
            filter=Q(
                votes__vote_type=Vote.DOWNVOTE,
            ),
        )
...
class CommentViewSet(viewsets.ModelViewSet, ReactionViewActionMixin):
    ...
    def get_queryset(self):
        ...
        comments = Comment.objects.filter(
            parent=thread_id, is_removed=is_removed
