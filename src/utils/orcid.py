...
from researchhub.settings import APP_ENV, PRODUCTION, SOCIALACCOUNT_PROVIDERS
from user.models.author import Author
from utils import sentry
from utils.exceptions import OrcidApiError
from utils.http import GET, POST, http_request
...
class OrcidApi:
    ...
    def get_authors(self, use_celery=PRODUCTION, **kwargs):
        ...
        try:
            ...
            for result in results:
                self._attempt_to_add_author(result, authors, use_celery=use_celery)
            return authors
    ...
    def _attempt_to_add_author(self, result, authors, attempts=2, use_celery=True):
        if use_celery:
            celery_add_author.apply_async(
                (result, authors, attempts), priority=4, countdown=5
            )
        else:
