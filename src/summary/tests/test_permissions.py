...
class SummaryPermissionsTests(TestCase):
    ...
    def test_can_patch_summary_with_minimum_reputation(self):
        user = create_random_authenticated_user_with_reputation(50, 50)
        response = self.get_summary_patch_response(user)
        self.assertEqual(response.status_code, 200)

    def test_can_NOT_patch_summary_below_minimum_reputation(self):
        user = create_random_authenticated_user_with_reputation(49, 49)
        response = self.get_summary_patch_response(user)
        self.assertEqual(response.status_code, 403)

    def test_can_put_summary_with_minimum_reputation(self):
        user = create_random_authenticated_user_with_reputation(50, 50)
        response = self.get_summary_put_response(user)
        self.assertEqual(response.status_code, 200)

    def test_can_NOT_put_summary_below_minimum_reputation(self):
        user = create_random_authenticated_user_with_reputation(49, 49)
        response = self.get_summary_put_response(user)
        self.assertEqual(response.status_code, 403)
    ...
    def get_summary_patch_response(self, user):
        url = self.base_url + f'{self.summary.id}/'
    ...
    def get_summary_put_response(self, user):
        url = self.base_url + f'{self.summary.id}/'
