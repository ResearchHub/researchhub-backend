...
class Command(BaseCommand):
    def handle(self, *args, **options):
        excluded_users = Balance.objects.values_list("user", flat=True).distinct()
        ...
        for i, user in enumerate(users):
            try:
                ...
                latest_distribution_id = (
                    user.reputation_records.order_by("-distributed_date").first().id
                )
    ...
    def _get_user_balance(self, user):
        ...
        unpaid_distributions = user.reputation_records.filter(
            paid_status=None, distributed_status=Distribution.DISTRIBUTED
        )
        return (sum([u_d.amount for u_d in unpaid_distributions]), unpaid_distributions)
