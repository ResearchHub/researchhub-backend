...
@receiver(post_save, sender=PaperVote, dispatch_uid="recalc_hot_score_on_paper_vote")
def recalc_hot_score(instance, sender, **kwargs):
    try:
        if type(instance) is Vote:
            ...
            recalc_hot_score_task.apply_async(
                (content_type_id, paper.id), priority=2, countdown=5
            )
...
def rh_unified_doc_sync_scores_paper(instance, sender, **kwargs):
    try:
        if sender is Paper and instance.unified_document is not None:
...
def sync_scores_uni_doc_and_paper(unified_doc, paper):
    ...
    if unified_doc.hot_score != paper.hot_score:
        ...
        should_save = True
    if unified_doc.score != paper.paper_score:
        unified_doc.score = paper.paper_score
        should_save = True
    if should_save:
        unified_doc.save()
...
@receiver(post_save, sender=Hypothesis, dispatch_uid="rh_unified_doc_sync_hypo_scores")
@receiver(
    post_save,
    sender=Vote,
