"""
User interaction event types for tracking and ML processing via Personalize.
"""

# Database event types
UPVOTE = "UPVOTE"
COMMENT_CREATED = "COMMENT_CREATED"
PEER_REVIEW_CREATED = "PEER_REVIEW_CREATED"
FEED_ITEM_CLICK = "FEED_ITEM_CLICK"
PAGE_VIEW = "PAGE_VIEW"
DOCUMENT_TAB_CLICKED = "DOCUMENT_TAB_CLICKED"
DOCUMENT_SAVED_TO_LIST = "DOCUMENT_SAVED_TO_LIST"
FEED_ITEM_ABSTRACT_EXPANDED = "FEED_ITEM_ABSTRACT_EXPANDED"
FEED_ITEM_IMPRESSION = "FEED_ITEM_IMPRESSION"

# Amplitude event types
FEED_ITEM_CLICKED = "feed_item_clicked"
PAGE_VIEWED = "work_document_viewed"
DOCUMENT_TAB_CLICKED_AMPLITUDE = "document_tab_clicked"
FEED_ITEM_ABSTRACT_EXPANDED_AMPLITUDE = "feed_item_abstract_expanded"
BULK_FEED_IMPRESSION = "bulk_feed_impression"

EVENT_CHOICES = [
    (UPVOTE, UPVOTE),
    (COMMENT_CREATED, COMMENT_CREATED),
    (PEER_REVIEW_CREATED, PEER_REVIEW_CREATED),
    (FEED_ITEM_CLICK, FEED_ITEM_CLICK),
    (PAGE_VIEW, PAGE_VIEW),
    (DOCUMENT_TAB_CLICKED, DOCUMENT_TAB_CLICKED),
    (DOCUMENT_SAVED_TO_LIST, DOCUMENT_SAVED_TO_LIST),
    (FEED_ITEM_ABSTRACT_EXPANDED, FEED_ITEM_ABSTRACT_EXPANDED),
    (FEED_ITEM_IMPRESSION, FEED_ITEM_IMPRESSION),
]

# Event Weights
EVENT_WEIGHTS = {
    UPVOTE: 8.0,
    COMMENT_CREATED: 12.0,
    PEER_REVIEW_CREATED: 16.0,
    FEED_ITEM_CLICK: 2.0,
    PAGE_VIEW: 1.0,
    DOCUMENT_TAB_CLICKED: 5.0,
    DOCUMENT_SAVED_TO_LIST: 10.0,
    FEED_ITEM_ABSTRACT_EXPANDED: 3.0,
    FEED_ITEM_IMPRESSION: 0.2,
}

# Mapping from Amplitude event types to database enum values
AMPLITUDE_TO_DB_EVENT_MAP = {
    FEED_ITEM_CLICKED: FEED_ITEM_CLICK,
    PAGE_VIEWED: PAGE_VIEW,
    DOCUMENT_TAB_CLICKED_AMPLITUDE: DOCUMENT_TAB_CLICKED,
    FEED_ITEM_ABSTRACT_EXPANDED_AMPLITUDE: FEED_ITEM_ABSTRACT_EXPANDED,
    BULK_FEED_IMPRESSION: FEED_ITEM_IMPRESSION,
}
