...
def _describe_matched_bounty(
    bounty: Bounty, hub: Hub, user_hub_score: int
) -> dict[str, Any]:
    """Build the payload the frontend renders for a bounty-for-you notification."""
    return {
        "bounty_id": bounty.id,
        "amount": bounty.amount,
        "bounty_type": bounty.bounty_type,
        "bounty_expiration_date": bounty.expiration_date,
        "user_hub_score": user_hub_score,
...
def find_qualified_users_and_notify(
    ...
    for author in qualified_authors:
        notification = notifications.send_once(
            ...
            item=bounty,
            unified_document=bounty.unified_document,
            extra=_describe_matched_bounty(
                bounty, hubs[author.matching_hub_id], author.max_hub_score
            ),
        )
...
def find_bounties_for_user_and_notify(user_id) -> Notification | None:
    ...
    notifications = NotificationService()
    hubs = Hub.objects.in_bulk({bounty.matching_hub_id for bounty in bounties})
    ...
    for bounty in bounties:
        notification = notifications.send_once(
            ...
            unified_document=bounty.unified_document,
            extra=_describe_matched_bounty(
                bounty, hubs[bounty.matching_hub_id], bounty.user_hub_score
            ),
        )
