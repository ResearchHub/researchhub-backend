...
class ListViewSetTests(APITestCase):
    ...
    def test_user_can_create_new_list(self):
    ...
    def test_creating_list_with_empty_name_shows_error(self):
        response = self.client.post("/api/user_list/", {"name": ""})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("error", response.data)
        self.assertIn("name", response.data["error"].lower())

    def test_creating_list_with_non_list_error_message_formats_correctly(self):
        with patch("user_lists.views.ListViewSet.get_serializer") as mock_get_serializer:
            mock_serializer = mock_get_serializer.return_value
            mock_serializer.is_valid.return_value = False
            mock_serializer.errors = {"name": "This field is required."}
            response = self.client.post("/api/user_list/", {"name": "Test"})
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
            self.assertIn("error", response.data)
            self.assertIn("name", response.data["error"].lower())
            self.assertIn("This field is required", response.data["error"])

    def test_creating_list_with_multiple_invalid_fields_shows_all_errors(self):
        response = self.client.post("/api/user_list/", {"name": "", "is_public": "invalid"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("error", response.data)
        self.assertIn("name", response.data["error"].lower())

    def test_creating_list_with_empty_error_messages_shows_generic_error(self):
        with patch("user_lists.views.ListViewSet.get_serializer") as mock_get_serializer:
            mock_serializer = mock_get_serializer.return_value
            mock_serializer.is_valid.return_value = False
    ...
    def test_updating_list_updates_timestamp_and_user(self):
    ...
    def test_deleting_list_also_deletes_all_items_but_preserves_already_removed_items(self):
        list_obj = List.objects.create(name="My List", created_by=self.user)
        item1 = ListItem.objects.create(parent_list=list_obj, unified_document=ResearchhubUnifiedDocument.objects.create(document_type=PAPER), created_by=self.user)
        item2 = ListItem.objects.create(parent_list=list_obj, unified_document=ResearchhubUnifiedDocument.objects.create(document_type=PAPER), created_by=self.user)
        item2.delete()
        item2_id = item2.id
        ...
        item2_refreshed = ListItem.all_objects.filter(id=item2_id).first()
        self.assertIsNotNone(item2_refreshed)
        self.assertTrue(item2_refreshed.is_removed)
...
class ListItemViewSetTests(APITestCase):
    ...
    def test_user_can_only_see_their_own_list_items(self):
    ...
    def test_retrieving_item_shows_detailed_serializer_with_unified_document_data(self):
    ...
    def test_listing_items_shows_detailed_serializer_with_unified_document_data(self):
        ListItem.objects.create(parent_list=self.list_obj, unified_document=self.doc, created_by=self.user)
        response = self.client.get("/api/user_list_item/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_creating_item_uses_basic_serializer_without_unified_document_data(self):
    ...
    def test_updating_item_uses_basic_serializer_without_unified_document_data(self):
        item = ListItem.objects.create(parent_list=self.list_obj, unified_document=self.doc, created_by=self.user)
    ...
    def test_creating_item_updates_parent_list_timestamp(self):
        response = self.client.post("/api/user_list_item/", {"parent_list": self.list_obj.id, "unified_document": self.doc.id})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(ListItem.objects.filter(parent_list=self.list_obj, unified_document=self.doc).exists())
    ...
    def test_updating_item_to_different_list_updates_both_list_timestamps(self):
        item = ListItem.objects.create(parent_list=self.list_obj, unified_document=self.doc, created_by=self.user)
    ...
    def test_serializing_item_with_exception_returns_minimal_data(self):
        item = ListItem.objects.create(parent_list=self.list_obj, unified_document=self.doc, created_by=self.user)
        with patch("user_lists.views.ListItemDetailSerializer") as mock_serializer_class:
    ...
    def test_user_can_add_new_item_to_list(self):
        other_doc = ResearchhubUnifiedDocument.objects.create(document_type=PAPER)
        response = self.client.post(
            "/api/user_list_item/add-item-to-list/",
