...
class ListViewSetTests(APITestCase):
    ...
    def test_user_can_create_list(self):
        ...
        self.assertTrue(
            List.objects.filter(name="My List", created_by=self.user).exists()
        )
    ...
    def test_user_can_create_multiple_lists_with_same_name(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            List.objects.filter(name="My List", created_by=self.user).count(), 2
        )
    ...
    def test_user_can_update_their_list(self):
        ...
        response = self.client.patch(
            f"/api/list/{list_obj.id}/", {"name": "Updated List"}
        )
    ...
    def test_user_cannot_update_another_users_list(self):
        ...
        response = self.client.patch(f"/api/list/{list_obj.id}/", {"name": "Hacked"})
    ...
    def test_user_can_retrieve_their_lists(self):
        ...
        ListItem.objects.create(
            parent_list=list1, unified_document=doc, created_by=self.user
        )
        ...
        list1_data = next(
            item for item in response.data["results"] if item["id"] == list1.id
        )
        self.assertEqual(list1_data["name"], "List 1")
        self.assertEqual(list1_data["item_count"], 1)

        list2_data = next(
            item for item in response.data["results"] if item["id"] == list2.id
        )
    ...
    def test_item_count_excludes_removed_items(self):
        ...
        item1 = ListItem.objects.create(
            parent_list=list_obj, unified_document=doc1, created_by=self.user
        )
        ListItem.objects.create(
            parent_list=list_obj, unified_document=doc2, created_by=self.user
        )

        item1.is_removed = True
        item1.save()

        response = self.client.get(f"/api/list/{list_obj.id}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["item_count"], 1)

    def test_user_can_delete_their_list(self):
        list_obj = List.objects.create(name="My List", created_by=self.user)
        response = self.client.delete(f"/api/list/{list_obj.id}/")
    ...
    def test_list_overview_returns_all_lists_with_documents(self):
        ...
        ListItem.objects.create(
            parent_list=list1, unified_document=doc1, created_by=self.user
        )
        ListItem.objects.create(
            parent_list=list1, unified_document=doc2, created_by=self.user
        )
        ListItem.objects.create(
            parent_list=list2, unified_document=doc3, created_by=self.user
        )
        ...
        list1_data = next(
            item for item in response.data["lists"] if item["list_id"] == list1.id
        )
        self.assertEqual(list1_data["name"], "List 1")
        self.assertEqual(len(list1_data["unified_documents"]), 2)
        list1_doc_ids = [
            item["unified_document_id"] for item in list1_data["unified_documents"]
        ]
        self.assertIn(doc1.id, list1_doc_ids)
        self.assertIn(doc2.id, list1_doc_ids)

        list2_data = next(
            item for item in response.data["lists"] if item["list_id"] == list2.id
        )
        self.assertEqual(list2_data["name"], "List 2")
        self.assertEqual(len(list2_data["unified_documents"]), 1)
        list2_doc_ids = [
            item["unified_document_id"] for item in list2_data["unified_documents"]
        ]
    ...
    def test_list_overview_excludes_removed_items(self):
        ...
        list_data = next(
            item for item in response.data["lists"] if item["list_id"] == list_obj.id
        )
        self.assertEqual(list_data["name"], "My List")
        self.assertEqual(len(list_data["unified_documents"]), 1)
        doc_ids = [
            item["unified_document_id"] for item in list_data["unified_documents"]
        ]
    ...
    def test_automatically_create_default_list_if_does_not_exist(self):
        ...
        response = self.client.post(
            "/api/list/default/item/", {"unified_document": paper.unified_document.id}
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        default_list = List.objects.filter(
    ...
    def test_multiple_items_are_added_to_default_list(self):
        ...
        self.client.post(
            "/api/list/default/item/", {"unified_document": paper1.unified_document.id}
        )
        self.client.post(
            "/api/list/default/item/", {"unified_document": paper2.unified_document.id}
        )

        default_lists = List.objects.filter(created_by=self.user, is_default=True)
        self.assertEqual(default_lists.count(), 1)
        self.assertEqual(
            default_lists.first().items.filter(is_removed=False).count(), 2
        )
    ...
    def test_overview_includes_default_list(self):
        ...
        default_list_data = next(
            (item for item in response.data["lists"] if item.get("is_default")), None
        )
    ...
    def test_adding_duplicate_document_fails(self):
        ...
        self.client.post(
            f"/api/list/{list_obj.id}/item/",
            {"parent_list": list_obj.id, "unified_document": paper.unified_document.id},
        )
        response = self.client.post(
            f"/api/list/{list_obj.id}/item/",
            {"parent_list": list_obj.id, "unified_document": paper.unified_document.id},
