...
class ListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            ...
            .annotate(item_count=Count("items", filter=Q(items__is_removed=False)))
    ...
    def overview(self, request):
        ...
        serializer = ListOverviewSerializer(
            lists, many=True, context=self.get_serializer_context()
        )
        return Response({"lists": serializer.data}, status=status.HTTP_200_OK)


class ListItemViewSet(
    CreateModelMixin, ListModelMixin, DestroyModelMixin, viewsets.GenericViewSet
):
    ...
    def get_queryset(self):
        ...
        qs = (
            self.queryset.filter(parent_list__created_by=self.request.user)
            .select_related("unified_document")
            .prefetch_related("unified_document__paper", "unified_document__posts")
        )
    ...
    def list(self, request, *args, **kwargs):
        ...
        check_list_exists = List.objects.filter(
            id=list_id, created_by=request.user, is_removed=False
        ).exists()
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not data.get("parent_list"):
            ...
            data = {
                "unified_document": data.get("unified_document"),
                "parent_list": default_list.id,
            }
