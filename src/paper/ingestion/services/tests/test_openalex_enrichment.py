...
class TestPaperOpenAlexEnrichmentService(TestCase):
    ...
    def test_process_authors_handles_duplicate_openalex_ids(self):
        """Test that process_authors handles multiple authors with the same openalex_ids."""
        from user.related_models.author_model import Author

        # Create two Author records with the same openalex_ids (simulating existing duplicates)
        ...
        # Mock mapper to return an author with the same openalex_ids
        mock_author = Mock(spec=Author)
        mock_author.openalex_ids = ["A999999"]
        mock_author.first_name = "Wei"
        mock_author.last_name = "Wang"
