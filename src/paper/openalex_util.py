...
from utils.http import clean_url
from utils.openalex import OpenAlex
from utils.openalex_service import OpenAlexService
...
def process_openalex_works(works):
    """
    Process OpenAlex works by creating authors, papers, authorships, and coauthors.

    This is a wrapper around the OpenAlexProcessor class that maintains backwards
...
def merge_openalex_author_with_researchhub_author(openalex_author, researchhub_author):
    processor = OpenAlexProcessor()
    processor.merge_openalex_author_with_researchhub_author(
        openalex_author, researchhub_author
    )


class OpenAlexProcessor:
    def __init__(self, openalex_service=None):
        """
        Initialize the OpenAlex processor.

        Args:
            openalex_service: Optional OpenAlexService instance for API interaction
        """
        self.openalex_service = openalex_service or OpenAlexService()

    def process_works(self, openalex_works):
        """
        Main entry point for processing OpenAlex works.

        This method orchestrates the entire workflow for processing works fetched from OpenAlex:
        1. Creating authors from work authorships
        2. Creating and updating papers based on work data
        3. Fetching detailed author information from OpenAlex
        4. Merging OpenAlex author data with ResearchHub authors
        5. Creating paper tags, authorships, institutions, and coauthor relationships

        Args:
            openalex_works (List[Dict]): List of work dictionaries from OpenAlex API

        Returns:
            None: The results are persisted to the database
        """
        researchhub_authors = self._create_authors(openalex_works)
        researchhub_authors_by_openalex_id = self._build_authors_by_oa_id_dict(
            researchhub_authors
        )

        paper_to_openalex_data = self._create_and_update_papers(openalex_works)

        detailed_author_data = self._fetch_authors_for_works(openalex_works)
        openalex_authors_by_work_id = self._build_oa_authors_by_work_id_dict(
            openalex_works, detailed_author_data
        )

        self._merge_authors(detailed_author_data, researchhub_authors_by_openalex_id)
        self._create_paper_tags(paper_to_openalex_data)
        self._create_authorships_and_institutions(
            paper_to_openalex_data,
            openalex_authors_by_work_id,
            researchhub_authors_by_openalex_id,
        )
        self._create_coauthors(
            paper_to_openalex_data,
            openalex_authors_by_work_id,
            researchhub_authors_by_openalex_id,
        )

    def _create_and_update_papers(self, openalex_works) -> Dict[int, Dict[str, Any]]:
        from paper.models import Paper

        dois = [work.get("doi") for work in openalex_works]
        dois = [doi for doi in dois if doi is not None]
        # if dois have https://doi.org/ prefix, remove them
        dois = [doi.replace("https://doi.org/", "") for doi in dois]
        openalex_ids = [work.get("id") for work in openalex_works]
        ...
        for work in openalex_works:
            ...
            if work.get("is_authors_truncated", False):
                just_id = work.get("id")
                work = self.openalex_service.get_work(just_id)
        ...
        paper_to_openalex_data = self._create_papers(papers_to_create.values())
        # Add updated papers to the dictionary
        paper_to_openalex_data.update(self._update_papers(papers_to_update.values()))

        return paper_to_openalex_data

    def _create_papers(self, works) -> Dict[int, Dict[str, Any]]:
        from paper.models import Paper
        ...
        for work in works:
            ...
            (
                ...
                openalex_concepts,
                openalex_topics,
            ) = self.openalex_service.build_paper_from_work(_work)
    ...
    def _update_papers(self, works) -> Dict[int, Dict[str, Any]]:
        from paper.models import Paper
    ...
    def _fetch_authors_for_works(self, openalex_works) -> List[Dict[str, Any]]:
        """
        Fetch detailed author information from OpenAlex for all authors in the provided works.

        This method:
        1. Extracts unique author IDs from all works
        2. Batches author ID requests to reduce API calls
        3. Fetches detailed author information from OpenAlex API

        Args:
            openalex_works (List[Dict]): List of works from OpenAlex API

        Returns:
            List[Dict[str, Any]]: List of detailed author dictionaries from OpenAlex

        Performance notes:
            - Uses batching to reduce API calls (100 authors per request)
            - Uses sets for efficient unique ID tracking
        """
        all_authors_to_fetch = set()
        batch_size = 100

        # Extract all unique author IDs from works
        for work in openalex_works:
        ...
        # Fetch authors in batches to reduce API calls
        fetched_oa_authors = []
        for i in range(0, len(all_authors_to_fetch), batch_size):
            batch = list(all_authors_to_fetch)[i : i + batch_size]
            oa_authors_batch, _ = self.openalex_service.get_authors(batch)
            fetched_oa_authors.extend(oa_authors_batch)

        return fetched_oa_authors

    def _build_oa_authors_by_work_id_dict(
        self, openalex_works, fetched_oa_authors
    ) -> Dict[str, List[Dict[str, Any]]]:
        fetched_oa_authors_by_id = {
            author["id"]: author for author in fetched_oa_authors
        }
    ...
    def _create_authors(self, works):
        from purchase.models import Wallet
        from user.related_models.author_model import Author

        # Get all authorships from the works
        openalex_authorships = [work.get("authorships", []) for work in works]
    ...
    def _build_authors_by_oa_id_dict(self, authors) -> Dict[str, List[Any]]:
        authors_by_oa_id = {}
    ...
    def _create_authorships_and_institutions(
        self, paper_to_openalex_data, oa_authors_by_work_id, authors_by_oa_id
    ):
        from institution.models import Institution
        ...
        for paper_id, paper_data in paper_to_openalex_data.items():
            ...
            for oa_authorship in openalex_authorships:
                ...
                for author in authors_for_oa_author:
                    ...
                    raw_author_name = oa_authorship.get("author", {}).get(
                        "display_name"
                    )
                    ...
                    for oa_inst in oa_authorship.get("institutions", []):
                        try:
                            ...
                            if institution:
                                ...
                                authorship_institution_relations[key].append(
                                    institution
                                )
                        except Exception as e:
                            sentry.log_error(
    ...
    def _create_paper_tags(self, papers_to_openalex_data):
    ...
    def _merge_authors(self, oa_authors, authors_by_oa_id):
        for oa_author in oa_authors:
            try:
                rh_authors = authors_by_oa_id.get(oa_author.get("id"), [])
                for rh_author in rh_authors:
                    self.merge_openalex_author_with_researchhub_author(
                        oa_author, rh_author
                    )
    ...
    def merge_openalex_author_with_researchhub_author(
        self, openalex_author, researchhub_author
    ):
        """
