...
class Command(BaseCommand):
    def handle(self, *args, **options):
        exclude_ids = (
            Figure.objects.all().distinct("paper").values_list("paper", flat=True)
        )
        papers = (
            Paper.objects.filter(file__isnull=False, created_date__gte="2020-12-20")
            .order_by("-id")
        ...
        for i, paper in enumerate(papers):
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"Failed to queue task for paper {paper.id}: {e}")
                )
