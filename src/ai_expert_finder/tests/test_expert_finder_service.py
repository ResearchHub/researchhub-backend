...
from ai_expert_finder.models import ExpertSearch
from ai_expert_finder.services.expert_finder_service import (
    ExpertFinderService,
...
from ai_expert_finder.services.openai_expert_finder_service import OPENAI_EXPERT_FINDER_MODEL
...
class GetDocumentContentTests(TestCase):
    ...
    @patch("ai_expert_finder.services.expert_finder_service._extract_text_from_pdf_bytes")
    @patch("ai_expert_finder.services.expert_finder_service._get_paper_pdf_bytes")
    ...
    def test_paper_pdf_too_large_raises(self, mock_get_pdf):
        from ai_expert_finder.constants import MAX_PDF_SIZE_BYTES
        from researchhub_document.related_models.constants.document_type import PAPER
...
class ExtractTextFromPdfBytesTests(TestCase):
    @patch("ai_expert_finder.services.expert_finder_service.fitz")
...
class GetPaperPdfBytesTests(TestCase):
    @patch("ai_expert_finder.services.expert_finder_service.download_pdf_from_url")
    @patch("ai_expert_finder.services.expert_finder_service.create_download_url")
    ...
    def test_falls_back_to_pdf_url_when_file_fails(self, mock_download):
        ...
        with patch(
            "ai_expert_finder.services.expert_finder_service.create_download_url",
...
class ExpertFinderServiceParseTests(TestCase):
    ...
    @patch("ai_expert_finder.services.expert_finder_service.OpenAIExpertFinderService")
    @patch("ai_expert_finder.services.expert_finder_service.ProgressService")
    @patch(
        "ai_expert_finder.services.expert_finder_service.generate_pdf_report",
        return_value=b"pdf",
    )
    @patch(
        "ai_expert_finder.services.expert_finder_service.generate_csv_file",
        return_value=b"csv",
    )
    @patch(
        "ai_expert_finder.services.expert_finder_service.upload_report_to_storage",
    ...
    def test_process_expert_search_stops_without_second_invoke_within_tolerance(
        ...
        from ai_expert_finder.services import expert_finder_service as efs
    ...
    def test_process_expert_search_error_message_truncated_when_llm_response_long(
        ...
        from ai_expert_finder.services.expert_finder_service import MAX_ERROR_MESSAGE_LENGTH
    ...
    def test_process_expert_search_expertise_level_string_and_list(
        ...
        from ai_expert_finder.constants import ExpertiseLevel
