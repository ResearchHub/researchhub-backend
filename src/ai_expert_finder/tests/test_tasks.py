"""Tests for ai_expert_finder.tasks: expert search, bulk email generation, send queued emails."""
...
from ai_expert_finder.tasks import (
...
class ProcessBulkGenerateEmailsTaskTests(TestCase):
    ...
    @patch("ai_expert_finder.tasks.generate_expert_email")
    ...
    @patch("ai_expert_finder.tasks.sentry")
    @patch("ai_expert_finder.tasks.generate_expert_email")
    ...
    def test_process_bulk_outer_exception_marks_all_failed_and_logs_sentry(
        ...
        with patch("ai_expert_finder.tasks.User.objects.get", side_effect=RuntimeError("DB error")):
            with self.assertRaises(RuntimeError):
