...
from ai_expert_finder.models import EmailTemplate, ExpertSearch, GeneratedEmail
from ai_expert_finder.services.email_sending_service import send_plain_email
from ai_expert_finder.views.email_views import _normalize_template
...
class GenerateEmailViewTests(APITestCase):
    def setUp(self):
        ...
        self.url = "/api/ai_expert_finder/expert-finder/generate-email/"
    ...
    @patch("ai_expert_finder.views.email_views.generate_expert_email")
...
class GeneratedEmailListViewTests(APITestCase):
    def setUp(self):
        self.moderator = create_random_authenticated_user("mod", moderator=True)
        self.user = create_random_authenticated_user("user", moderator=False)
        self.url = "/api/ai_expert_finder/expert-finder/emails/"
...
class GeneratedEmailDetailViewTests(APITestCase):
    ...
    def test_get_requires_authentication(self):
        ...
        response = self.client.get(
            f"/api/ai_expert_finder/expert-finder/emails/{email.id}/"
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_get_list_navigation_matches_list_order_for_search(self):
        ...
        r_mid = self.client.get(f"/api/ai_expert_finder/expert-finder/emails/{mid.id}/")
        ...
        r_new = self.client.get(f"/api/ai_expert_finder/expert-finder/emails/{newest.id}/")
        ...
        r_old = self.client.get(f"/api/ai_expert_finder/expert-finder/emails/{oldest.id}/")
    ...
    def test_get_list_navigation_without_expert_search_uses_global_sequence(self):
        ...
        self.client.force_authenticate(self.moderator)
        r = self.client.get(f"/api/ai_expert_finder/expert-finder/emails/{second.id}/")
    ...
    def test_get_returns_404_for_nonexistent(self):
        self.client.force_authenticate(self.moderator)
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/emails/999999/"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class BulkGenerateEmailViewTests(APITestCase):
    def setUp(self):
        self.moderator = create_random_authenticated_user("mod", moderator=True)
        self.url = "/api/ai_expert_finder/expert-finder/generate-emails-bulk/"
        self.expert_search = _make_expert_search(
            self.moderator,
            expert_results=[
                {
    ...
    @patch("ai_expert_finder.views.email_views.process_bulk_generate_emails_task")
...
class SendPlainEmailTests(APITestCase):
    ...
    @patch("ai_expert_finder.services.email_sending_service.send_mail")
    ...
    @patch("ai_expert_finder.services.email_sending_service.EmailMultiAlternatives")
...
class PreviewEmailViewTests(APITestCase):
    def setUp(self):
        self.moderator = create_random_authenticated_user("mod", moderator=True)
        self.url = "/api/ai_expert_finder/expert-finder/emails/preview/"
    ...
    @patch("ai_expert_finder.views.email_views.send_plain_email")
...
class SendEmailViewTests(APITestCase):
    def setUp(self):
        self.moderator = create_random_authenticated_user("mod", moderator=True)
        self.url = "/api/ai_expert_finder/expert-finder/emails/send/"
    ...
    @patch("ai_expert_finder.views.email_views.send_queued_emails_task")
    ...
    @patch("ai_expert_finder.tasks.send_plain_email")
    def test_send_queued_emails_task_sends_and_updates_status(self, mock_send):
        from ai_expert_finder.tasks import send_queued_emails_task
