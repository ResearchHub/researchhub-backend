...
from ai_expert_finder.models import DocumentInvitedExpert, ExpertSearch
...
class ExpertSearchCreateViewTests(APITestCase):
    def setUp(self):
        ...
        self.url = "/api/ai_expert_finder/expert-finder/search/"
    ...
    @patch("ai_expert_finder.tasks.process_expert_search_task.delay")
    ...
    @patch("ai_expert_finder.views.expert_finder_views.get_document_content")
    @patch("ai_expert_finder.tasks.process_expert_search_task.delay")
    ...
    @patch("ai_expert_finder.tasks.ExpertFinderService")
...
class ExpertSearchDetailViewTests(APITestCase):
    def setUp(self):
        ...
        self.url = "/api/ai_expert_finder/expert-finder/search/{}/".format(
            self.search.id
        )
    ...
    def test_get_detail_returns_work_when_unified_document(self):
        ...
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/search/{}/".format(search.id)
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
    ...
    def test_get_non_integer_search_id_returns_404(self):
        ...
        self.client.force_authenticate(self.moderator)
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/search/abc/"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class ExpertSearchListViewTests(APITestCase):
    def setUp(self):
        ...
        self.url = "/api/ai_expert_finder/expert-finder/searches/"
...
class ExpertSearchProgressStreamViewTests(APITestCase):
    def setUp(self):
        ...
        self.url = "/api/ai_expert_finder/expert-finder/progress/{}/".format(
            self.search.id
        )
...
class ExpertSearchWorkViewTests(APITestCase):
    ...
    def test_work_requires_authentication(self):
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/work/1/"
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_work_unified_document_not_found_returns_404(self):
        self.client.force_authenticate(self.moderator)
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/work/999999/"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_work_returns_paper_when_unified_document_is_paper(self):
        ...
        self.client.force_authenticate(self.moderator)
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/work/{}/".format(
    ...
    @patch("ai_expert_finder.views.expert_finder_views.resolve_work_for_unified_document")
...
class InvitedExpertsDocumentViewTests(APITestCase):
    ...
    def test_invited_requires_authentication(self):
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/documents/1/invited/"
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_invited_nonexistent_document_returns_404(self):
        self.client.force_authenticate(self.moderator)
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/documents/999999/invited/"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(
    ...
    def test_invited_valid_document_returns_200_and_structure(self):
        ...
        self.client.force_authenticate(self.moderator)
        response = self.client.get(
            "/api/ai_expert_finder/expert-finder/documents/{}/invited/".format(
                paper.unified_document_id
            )
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
