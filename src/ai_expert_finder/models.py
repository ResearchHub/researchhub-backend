...
from ai_expert_finder.constants import EmailTemplateType
...
class ExpertSearch(DefaultModel):
    ...
    class Meta:
        db_table = "ai_expert_finder_expert_search"
        ordering = ["-created_date"]
        indexes = [
            models.Index(
                fields=["created_by", "status"],
                name="aef_es_user_status",
            ),
            models.Index(fields=["status"], name="aef_es_status"),
        ]
...
class GeneratedEmail(DefaultModel):
    ...
    class Meta:
        db_table = "ai_expert_finder_generated_email"
        ordering = ["-created_date"]
        indexes = [
            models.Index(
                fields=["created_by"],
                name="aef_ge_created_by",
            ),
        ]
...
class DocumentInvitedExpert(DefaultModel):
    ...
    class Meta:
        db_table = "ai_expert_finder_document_invited_expert"
        ordering = ["-created_date"]
        constraints = [
            models.UniqueConstraint(
                fields=["unified_document", "user"],
                name="aef_die_doc_user_unique",
            ),
        ]
        indexes = [
            models.Index(
                fields=["unified_document"],
                name="aef_die_unified_doc",
            ),
            models.Index(
                fields=["expert_search"],
                name="aef_die_expert_search",
            ),
        ]
...
class EmailTemplate(DefaultModel):
    ...
    class Meta:
        db_table = "ai_expert_finder_email_template"
        ordering = ["-updated_date"]
        indexes = [
            models.Index(
                fields=["created_by"],
                name="aef_et_created_by",
            ),
        ]
