...
class ProposalKeyInsightsService:
    """Generate proposal key insights (TLDR + strengths/weaknesses) via Bedrock."""

    _MAX_HUMAN_REVIEWS_TOTAL = 30000
    _MAX_LLM_TOKENS = 4096

    def __init__(self, llm: BedrockLLMService | None = None) -> None:
        self._llm = llm
    ...
    def _get_rhf_endorsed_human_reviews(
        ...
        out = "\n\n".join(blocks).strip()
        cap = ProposalKeyInsightsService._MAX_HUMAN_REVIEWS_TOTAL
        if len(out) > cap:
    ...
    def _parse_strengths_weaknesses(
        ...
        return (
            ProposalKeyInsightsService._row_list(data.get("strengths")),
            ProposalKeyInsightsService._row_list(data.get("weaknesses")),
        )
    ...
    def run(self, review_id: int, *, force: bool = False) -> ProposalKeyInsight:
        """
        Generate and store proposal **key insights** (TLDR plus strengths/weaknesses).

        Requires ``ProposalReview`` to have finished the main AI review
        (``status == Status.COMPLETED``). Calls Bedrock with a prompt built from:

        - Proposal body (markdown),
        - Optional RFP / funding-opportunity (grant) context when the review is linked
          to a grant,
        - The platform AI peer-review comment for this review (plain text),
        - Assessed human community reviews on the proposal post.

        Parses the model JSON (``tldr``, ``strengths``, ``weaknesses``), then persists
        ``ProposalKeyInsight`` and related ``ProposalKeyInsightItem`` rows. With
        ``force=False``, skips the LLM when a completed key insight already exists.
        """
        ...
        if review.status != Status.COMPLETED:
            raise ValueError(
                f"Proposal review {review_id} must be completed "
        ...
        if key_insight.status == Status.COMPLETED and not force:
            return key_insight

        key_insight.status = Status.PROCESSING
        ...
        try:
            ...
            strengths, weaknesses = self._parse_strengths_weaknesses(data)
            ...
            with transaction.atomic():
                ...
                key_insight.status = Status.COMPLETED
                key_insight.error_message = ""
                key_insight.save(
                    update_fields=[
                        "tldr",
