...
def recompute_overall_fields(review_dict: dict) -> None:
    """
    Set ``overall_rating`` / ``overall_score_numeric`` from the LLM after canonicalization.

    When the LLM omits or sends an unusable ``overall_rating``, it is set to ``None``.
    When ``overall_score_numeric`` is missing or unusable, it defaults to ``1`` (minimum
    on the 1-3 scale) so the field is always a valid integer for sorting and display.
    """
    review_dict["overall_rating"] = _canonical_overall_rating(
        review_dict.get("overall_rating")
    )
    raw_num = review_dict.get("overall_score_numeric")
    if isinstance(raw_num, bool):
        raw_num = None
    num = _canonical_overall_score_numeric(raw_num)
    review_dict["overall_score_numeric"] = 1 if num is None else num
