...
class OrcidCallbackServiceTests(TestCase):
    ...
    def test_process_callback_success(self):
        ...
        self.mock_client.exchange_code_for_token.return_value = {
            "orcid": OrcidTestHelper.ORCID_ID,
            "access_token": "tk",
            "refresh_token": "rt",
            "expires_in": 3600,
        }
        state = signing.dumps(
            {"user_id": user.id, "return_url": "https://researchhub.com/p"}
        )
        ...
        self.assertTrue(
            SocialAccount.objects.filter(user=user, provider=OrcidProvider.id).exists()
        )
    ...
    def test_process_callback_already_linked(self):
        ...
        SocialAccount.objects.create(
            user=user1, provider=OrcidProvider.id, uid=OrcidTestHelper.ORCID_ID
        )
        self.mock_client.exchange_code_for_token.return_value = {
            "orcid": OrcidTestHelper.ORCID_ID
        }
    ...
    def test_get_redirect_url(self):
        ...
        success = self.service.get_redirect_url(
            return_url="https://researchhub.com?foo=bar"
        )
