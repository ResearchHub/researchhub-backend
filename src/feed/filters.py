...
class FundOrderingFilter(OrderingFilter):
    ...
    def filter_queryset(
        ...
        queryset = self._apply_include_ended_filter(
            request, queryset, view, model_config
        )
    ...
    def _apply_include_ended_filter(
        self,
        request: Request,
        queryset: QuerySet,
        view: Any,
        ...
        include_ended_filter_value = request.query_params.get(
            "include_ended", "true"
        ).upper()
        should_apply_filter = (
            include_ended_filter_value == "TRUE"
            or fundraise_status_filter_value == "CLOSED"
        )
        ...
        return queryset.exclude(
            unified_document__in=model_class.objects.filter(
                status=open_status, end_date__lt=now
    ...
    def _apply_custom_sorting(
        ...
        if ordering == "newest":
            ...
            return self._apply_most_applicants_sorting(
                queryset, model_config["model_class"]
            )
        elif ordering == "amount_raised":
            return self._apply_amount_raised_sorting(
                queryset, model_config["model_class"]
            )
        else:
    ...
    def _apply_newest_sorting(
        ...
        open_status = model_config["open_status"]
        closed_statuses = [
            model_config["closed_status"],
            model_config["completed_status"],
        ]
        ...
        has_open_item = Exists(
            model_class.objects.filter(
                unified_document_id=OuterRef("unified_document_id"), status=open_status
            )
        )
        ...
        queryset = queryset.annotate(
            ...
            earliest_open_end_date=Subquery(
                earliest_open_end_date, output_field=DateTimeField()
            ),
            latest_closed_end_date=Subquery(
                latest_closed_end_date, output_field=DateTimeField()
            ),
            sort_option=Case(
    ...
    def _apply_best_sorting(
        ...
        queryset = queryset.annotate(
            ...
            sort_option=Case(
                When(
                    has_open=True, earliest_open_end_date__gte=now, then=Value(0)
                ),  # Active
                When(
                    has_open=True, earliest_open_end_date__isnull=True, then=Value(0)
                ),  # Active (no end date)
                When(
                    has_open=True, earliest_open_end_date__lt=now, then=Value(1)
                ),  # Expired
        ...
        queryset = queryset.annotate(
            amount=Case(
                When(
                    sort_option=0, then=amount_expr
                ),  # Only active items sorted by amount
                default=Value(0),
                output_field=DecimalField(max_digits=19, decimal_places=10),
            ),
        )
        ...
        if model_class == Grant:
            ...
            return queryset.annotate(
                contributor_count=Count(
                    "unified_document__fundraises__purchases__user", distinct=True
                )
