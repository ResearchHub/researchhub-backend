"""
DEPRECATED Hot Score Implementation

This is the original hot score algorithm, kept for comparison and rollback
purposes. Use hot_score.py for the new implementation.

TODO: Remove this file once hot_score_v2 is validated and fully deployed.
"""
...
def calculate_hot_score_for_item_DEPRECATED(feed_entry):
    """
    DEPRECATED: Calculate hot score for an item using old algorithm.
    Use calculate_hot_score_for_item() from hot_score.py instead.
    """
    ...
    if item_content_type == ContentType.objects.get_for_model(RhCommentModel) and (
        ...
        return calculate_hot_score_for_peer_review_DEPRECATED(feed_entry)
    elif item_content_type == ContentType.objects.get_for_model(
        ResearchhubPost
    ) or item_content_type == ContentType.objects.get_for_model(Paper):
        return calculate_hot_score_DEPRECATED(feed_entry, item_content_type)
    else:
        return 0


def calculate_hot_score_for_peer_review_DEPRECATED(feed_entry):
    """
    DEPRECATED: Calculate hot score for a peer review using old algorithm.
    Use calculate_hot_score_for_peer_review() from hot_score.py instead.
    """
    ...
    peer_review_score = calculate_hot_score_DEPRECATED(
        feed_entry,
        ContentType.objects.get_for_model(RhCommentModel),
    )

    # Add the peer review's own score to ensure it ranks higher
...
def calculate_hot_score_DEPRECATED(feed_entry, content_type_name):
    """
    DEPRECATED: Calculate hot score for a feed entry based on its content
    type.
