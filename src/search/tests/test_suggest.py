...
@patch("utils.openalex.OpenAlex.autocomplete_works")
@patch("opensearchpy.Search.execute")
def test_verified_user_prioritization(self, mock_es_execute, mock_openalex):
    """Test that verified users appear first in search results"""
    # Mock empty OpenAlex response since we're testing user search
    mock_openalex.return_value = {"results": []}

    # Mock Elasticsearch response with both verified and unverified users
    user_options = [
        {
            "_score": 1.0,
            "_source": {
                "id": 1,
                "full_name": "Nick Tytarenko",
                "created_date": "2023-01-01",
        ...
        },
        {
            ...
            "_source": {
                ...
                "full_name": "Nick Tytarenko",
                "created_date": "2023-01-01",
                "is_verified": False,  # Another unverified user
            },
        },
    ]
    ...
    # Test search for "nick tytarenko"
    response = self.client.get(self.url + "?q=nick tytarenko&index=user")
    self.assertEqual(response.status_code, status.HTTP_200_OK)

    # Should have 3 results
    self.assertEqual(len(response.data), 3)

    # First result should be the verified user
    first_result = response.data[0]
    self.assertEqual(first_result["entity_type"], "user")
    self.assertTrue(first_result["is_verified"], "First result should be verified")
    self.assertEqual(first_result["id"], 2)

    # Verify the boost type is correct
    self.assertEqual(first_result["_boost"], "verified_user")

    # All results should have the same name
    for result in response.data:
        self.assertEqual(result["display_name"], "Nick Tytarenko")


@patch("utils.openalex.OpenAlex.autocomplete_works")
@patch("opensearchpy.Search.execute")
def test_verified_user_with_exact_match(self, mock_es_execute, mock_openalex):
    """Test that verified users with exact name matches get highest priority"""
    # Mock empty OpenAlex response
    mock_openalex.return_value = {"results": []}

    # Mock Elasticsearch response with verified and unverified users
    user_options = [
        {
            "_score": 1.0,
            "_source": {
                ...
                "created_date": "2023-01-01",
                "is_verified": False,  # Unverified exact match
            },
        },
        {
            "_score": 1.0,
            "_source": {
                ...
                "full_name": "John Smith",
                "created_date": "2023-01-01",
                "is_verified": True,  # Verified exact match
            },
        },
        {
            "_score": 1.0,
            "_source": {
                ...
                "created_date": "2023-01-01",
                "is_verified": True,  # Verified partial match
            },
        },
    ]
    ...
    # Test search for exact name "John Smith"
    response = self.client.get(self.url + "?q=John Smith&index=user")
    self.assertEqual(response.status_code, status.HTTP_200_OK)

    # Should have 3 results
    self.assertEqual(len(response.data), 3)

    # First result should be verified user with exact match
    first_result = response.data[0]
    self.assertEqual(first_result["entity_type"], "user")
    self.assertTrue(first_result["is_verified"])
    self.assertEqual(first_result["display_name"], "John Smith")
    self.assertEqual(first_result["_boost"], "verified_user")

    # Second result should be unverified user with exact match
    second_result = response.data[1]
    self.assertFalse(second_result["is_verified"])
    self.assertEqual(second_result["display_name"], "John Smith")
    self.assertEqual(second_result["_boost"], "exact_name_match")

    # Third result should be verified user with partial match
    third_result = response.data[2]
    self.assertTrue(third_result["is_verified"])
    self.assertEqual(third_result["display_name"], "Johnny Smith")
    self.assertEqual(third_result["_boost"], "partial_name_match")


@patch("utils.openalex.OpenAlex.autocomplete_works")
@patch("opensearchpy.Search.execute")
def test_verified_user_scoring_priority(self, mock_es_execute, mock_openalex):
    """Test that verification status overrides other scoring factors"""
    # Mock empty OpenAlex response
    mock_openalex.return_value = {"results": []}

    # Mock Elasticsearch response with users having different base scores
    user_options = [
        {
            "_score": 10.0,  # High base score
            "_source": {
                "id": 1,
                "full_name": "Alice Johnson",
                "created_date": "2023-01-01",
                "is_verified": False,  # Unverified but high score
            },
        },
        {
            "_score": 1.0,  # Low base score
            "_source": {
                "id": 2,
                "full_name": "Alice Johnson",
                "created_date": "2023-01-01",
                "is_verified": True,  # Verified but low score
            },
        },
        {
            "_score": 5.0,  # Medium base score
            "_source": {
                "id": 3,
                "full_name": "Alice Johnson",
                "created_date": "2023-01-01",
                "is_verified": False,  # Unverified with medium score
            },
        },
    ]
    ...
    # Test search for "Alice Johnson"
    response = self.client.get(self.url + "?q=Alice Johnson&index=user")
    self.assertEqual(response.status_code, status.HTTP_200_OK)

    # Should have 3 results
    self.assertEqual(len(response.data), 3)

    # First result should be verified user despite having lowest base score
    first_result = response.data[0]
    self.assertTrue(first_result["is_verified"])
    self.assertEqual(first_result["id"], 2)
    self.assertEqual(first_result["_boost"], "verified_user")

    # Verify the scoring order: verified first, then by base score
    self.assertEqual(response.data[0]["id"], 2)  # Verified (low score)
    self.assertEqual(response.data[1]["id"], 1)  # Unverified (high score)
    self.assertEqual(response.data[2]["id"], 3)  # Unverified (medium score)


@patch("utils.openalex.OpenAlex.autocomplete_works")
@patch("opensearchpy.Search.execute")
def test_verified_user_with_mixed_entity_types(self, mock_es_execute, mock_openalex):
    """Test that verified users are prioritized even when mixed with other entity types"""
    # Mock empty OpenAlex response
    mock_openalex.return_value = {"results": []}

    # Mock Elasticsearch responses for different indexes
    def mock_execute_side_effect(search):
        try:
            current_index = search.index._name
            if "user" in current_index:
                # User results with verified user
                user_options = [
                    {
                        "_score": 1.0,
                        "_source": {
                ...
                hub_options = [
                    {
                        "_score": 10.0,  # High score hub
                        "_source": {
                            "id": 3,
                            "name": "Bob's Research Hub",
                            "slug": "bobs-research-hub",
                            "description": "A research hub",
                            "paper_count": 100,
                            "discussion_count": 50,
                            "created_date": "2023-01-01",
                        },
    ...
    # Test search across user and hub indexes
    response = self.client.get(self.url + "?q=Bob&index=user,hub")
    self.assertEqual(response.status_code, status.HTTP_200_OK)

    # Should have results from both indexes
    self.assertGreater(len(response.data), 0)

    # Find the verified user result
    verified_user = next(
        (
            r
            for r in response.data
            if r.get("is_verified") and r["entity_type"] == "user"
        ),
        None,
    )
    self.assertIsNotNone(verified_user, "Should find verified user in results")
    self.assertEqual(verified_user["display_name"], "Bob Wilson")
    self.assertEqual(verified_user["_boost"], "verified_user")

    # Find the hub result
    hub_result = next((r for r in response.data if r["entity_type"] == "hub"), None)
    self.assertIsNotNone(hub_result, "Should find hub in results")
    self.assertEqual(hub_result["display_name"], "Bob's Research Hub")


@patch("utils.openalex.OpenAlex.autocomplete_works")
@patch("opensearchpy.Search.execute")
def test_verified_user_field_in_response(self, mock_es_execute, mock_openalex):
    """Test that the is_verified field is properly included in user search results"""
    # Mock empty OpenAlex response
    mock_openalex.return_value = {"results": []}

    # Mock Elasticsearch response with verified user
    user_options = [
        {
            "_score": 1.0,
            "_source": {
    ...
    # Verify the is_verified field is present and correct
    result = response.data[0]
...
@patch("utils.openalex.OpenAlex.autocomplete_works")
@patch("opensearchpy.Search.execute")
def test_unverified_user_handling(self, mock_es_execute, mock_openalex):
    """Test that unverified users are handled correctly and don't get verification boost"""
    # Mock empty OpenAlex response
    mock_openalex.return_value = {"results": []}

    # Mock Elasticsearch response with unverified user
    user_options = [
        {
            "_score": 1.0,
            "_source": {
    ...
    # Test search for unverified user
    response = self.client.get(self.url + "?q=Unverified&index=user")
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    self.assertEqual(result["entity_type"], "user")
    self.assertEqual(result["display_name"], "Unverified User")

    # Should not have verified_user boost
    self.assertNotEqual(result.get("_boost"), "verified_user")
