...
def log_search_error(
    ...
    """Log search view errors with enhanced details.

    Called from exception handlers - if this function itself raises an exception,
    it will be automatically captured by the logging system.
    ...
    if opensearch_details:
        extra_data["opensearch_details"] = opensearch_details
    logger.error(
        "Unified search error: %s", exception_type, extra=extra_data, exc_info=True
    )
