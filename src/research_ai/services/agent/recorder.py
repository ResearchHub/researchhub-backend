...
class AgentRecorder(Protocol):
    ...
    def is_active(self) -> bool:
        ...
        Optional. Recorders that can be terminated from outside the run --
        cancelled, or reclaimed as abandoned -- implement this so the loop can
        stop *before* a tool call takes effect rather than discovering it at the
        next write, which happens only after the side effect. A recorder without
        it is treated as always active, so nothing is required of observers that
