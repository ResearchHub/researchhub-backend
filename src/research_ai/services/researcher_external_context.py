...
def build_researcher_external_context_for_author(
    ...
    *,
    ...
    if author is None:
        return ""
    _, orcid = normalize_orcid(getattr(author, "orcid_id", None))
...
def build_researcher_external_context(
    ...
    *,
    ...
    try:
        _, orcid_bare = normalize_orcid(author.orcid_id)
        works_text = build_orcid_works_context_text(orcid_bare=orcid_bare)
        if works_text.strip():
            chunks.append(
