...
2. **name+affiliation** -- the affiliation string is resolved to an OpenAlex
   *institution* (OpenAlex's own entity resolution handles abbreviations,
   ...
   institution id.
3. **name** -- a single, exact full-name match with nothing to disambiguate
   against.
4. **llm-adjudicated** -- several plausible candidates (or one non-exact match)
   go to a conservative LLM verdict (``adjudication.pick_candidate``).

Note the expert's ``affiliation``/``expertise`` are machine-generated by the
expert finder, not self-reported -- both sides of the match are noisy, which is
why every rung prefers a miss over a wrong match.
"""
...
def resolve_openalex_author(
    ...
    if len(scored) == 1 and scored[0][0] >= 1.0:
    ...
    # 4. Ambiguous middle: several plausible candidates (or one non-exact).
    try:
        record, confidence = pick_candidate(
            expert, [rec for _, rec in scored], service=adjudication_service
        )
    except Exception as exc:  # noqa: BLE001 - adjudication is best-effort
        logger.info("LLM adjudication failed for %r: %s", name, exc)
        return AuthorResolution(
